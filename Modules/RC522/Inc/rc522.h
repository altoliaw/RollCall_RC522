...
#define PICC_REQIDL     ((uint8_t)0x26U) /**< REQA: request idle (non-halted) cards only. */
#define PICC_REQALL     ((uint8_t)0x52U) /**< WUPA: request all cards, including halted ones. */
#define PICC_ANTICOLL   ((uint8_t)0x93U) /**< Anti-collision, cascade level 1. */
#define PICC_SELECTTAG  ((uint8_t)0x93U) /**< Select tag, cascade level 1. */
#define PICC_HALT       ((uint8_t)0x50U) /**< HLTA: instruct the selected card to enter the HALT state. */

/* Exported PCD (reader-side) command codes -------------------------------- */
#define PCD_RESETPHASE  ((uint8_t)0x0FU) /**< Perform a soft reset of the MFRC522. */
#define PCD_TRANSCEIVE  ((uint8_t)0x0CU) /**< Transmit FIFO data and automatically receive the reply. */
#define PCD_CALCCRC     ((uint8_t)0x03U) /**< Activate the internal CRC coprocessor. */
 ...
 * @param   addr  Target register address, as defined in the MFRC522
 *                datasheet, Section 9.3 (Register Overview).
 * @param   val   Byte value to be written into the target register.
 * @return  HAL_OK if the SPI transaction completed successfully;
 ...
 *          field.
 * @param   reqMode  Either PICC_REQIDL (idle cards only) or PICC_REQALL
